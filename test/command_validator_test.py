...
class CommandValidatorTest(BaseConnorTestCase):
    def test_Validations(self):
        function_names = [f.__name__ for f in command_validator._VALIDATIONS]
    ...
    def test_preflight_runsAllValidations(self):
        task1 = MockTask()
        task2 = MockTask()
        command_validator._VALIDATIONS = [task1.execute,
                                          task2.execute]
        args = Namespace()
    ...
    def test_input_bam_exists(self):
        with TempDirectory() as tmp_dir:
            ...
            command_validator._check_input_bam_exists(args)
            self.ok()
    ...
    def test_input_bam_exists_raisesUsageError(self):
        with TempDirectory() as tmp_dir:
            ...
            self.assertRaisesRegexp(utils.UsageError,
                                    ...
                                    command_validator._check_input_bam_exists,
                                    args)
...
readNameA1|99|chr10|100|20|5M|=|300|200|AAAAA|>>>>>'''.replace("|", "\t")
        with TempDirectory() as tmp_dir:
            input_bam_path = samtools_test.create_bam(tmp_dir.path,
                                                      ...
                                                      index=False)
            args = Namespace(input_bam=input_bam_path)
            command_validator._check_input_bam_valid(args)
            self.ok()
...
readNameA1|99|chr10|100|20|5M|=|300|200|AAAAA|>>>>>'''.replace("|", "\t")
        with TempDirectory() as tmp_dir:
            input_bam_path = samtools_test.create_bam(tmp_dir.path,
                                                      ...
                                                      index=True)
            args = Namespace(input_bam=input_bam_path)
            command_validator._check_input_bam_indexed(args)
            self.ok()
...
readNameA1|99|chr10|100|20|5M|=|300|200|AAAAA|>>>>>'''.replace("|", "\t")
        with TempDirectory() as tmp_dir:
            ...
            command_validator._check_input_bam_not_connor_generated(args)
            self.ok()
...
readNameA1|99|chr10|100|20|5M|=|300|200|AAAAA|>>>>>'''.replace("|", "\t")
        with TempDirectory() as tmp_dir:
            ...
            self.assertRaisesRegexp(utils.UsageError,
                                    regex,
                                    command_validator._check_input_bam_not_connor_generated,
                                    args)
...
readNameA1|99|chr10|100|20|5M|=|300|200|AAAAA|>>>>>'''.replace("|", "\t")
        with TempDirectory() as tmp_dir:
            input_bam_path = samtools_test.create_bam(tmp_dir.path,
                                                      ...
                                                      index=True)
            args = Namespace(input_bam=input_bam_path, force=False)
            command_validator._check_input_bam_not_empty(args)
            self.ok()
...
readNameA1|{flag}|chr10|100|20|5M|=|300|200|AAAAA|>>>>>'''
        ...
        with TempDirectory() as tmp_dir:
            input_bam_path = samtools_test.create_bam(tmp_dir.path,
                                                      ...
                                                      index=True)
            args = Namespace(input_bam=input_bam_path, force=False)
            regex = r'\[.*input.bam\] does not contain paired reads'
            self.assertRaisesRegexp(utils.UsageError,
                                    regex,
                                    command_validator._check_input_bam_paired,
                                    args)
...
readNameA1|{paired_flag}|chr10|100|20|5M|=|300|200|AAAAA|>>>>>'''
        ...
        with TempDirectory() as tmp_dir:
            input_bam_path = samtools_test.create_bam(tmp_dir.path,
                                                      ...
                                                      index=True)
            args = Namespace(input_bam=input_bam_path, force=False)
            command_validator._check_input_bam_paired(args)
            self.ok()
...
'''
        ...
        with TempDirectory() as tmp_dir:
            input_bam_path = samtools_test.create_bam(tmp_dir.path,
                                                      ...
                                                      index=True)
            args = Namespace(input_bam=input_bam_path, force=False)
            command_validator._check_input_bam_barcoded(args)
            self.ok()
...
'''
    ...
    def test_check_input_bam_barcoded_leftUnbarcoded_raisesUsageError(self):
        sam_contents = \
'''@HD|VN:1.4|GO:none|SO:coordinate
@SQ|SN:chr10|LN:135534747
readNameA1|99|chr10|100|20|8M|=|300|200|NNNAAAAA|>>>>>>>>
...
'''
        ...
        with TempDirectory() as tmp_dir:
            ...
            self.assertRaisesRegexp(utils.UsageError,
                                    regex,
                                    command_validator._check_input_bam_barcoded,
                                    args)

    def test_check_input_bam_barcoded_rightUnbarcoded_raisesUsageError(self):
        sam_contents = \
'''@HD|VN:1.4|GO:none|SO:coordinate
@SQ|SN:chr10|LN:135534747
readNameA1|99|chr10|100|20|8M|=|300|200|NNNAAAAA|>>>>>>>>
